...
{
    public function chartController($type = 'student'){
...
$eSubQuery = DB::table('questions as a')
    ...
    ->select(
        's.abbrev',
        DB::raw('COUNT(a.employee_id) as high_count ')
    )
    ->where('a.status', '=', $highestStatusE)  // Filter by Moderate status
    ->groupBy('s.abbrev')
    ->orderBy('high_count', 'DESC');  // Sort by the count in descending order
...
foreach ($empData as $row) {
    ...
    $empChartData['datasets'][0]['data'][] = $row->high_count;
}
